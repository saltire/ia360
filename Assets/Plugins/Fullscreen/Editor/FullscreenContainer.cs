...
namespace FullscreenEditor
{
    ...
    public abstract partial class FullscreenContainer : HostView
    {
        ...
        {
            get => EditorPrefs.GetInt("FullscreenIdx", 0);
        ...
        {
            get =>
                m_dst.Container ? m_dst.Container.GetPropertyValue<Rect>("position") : new Rect();
            set
            {
                if (m_dst.Container)
                {
                ...
                {
                    Logger.Debug("No container on {0}, rect will not be set", name);
                }
            }
        }
