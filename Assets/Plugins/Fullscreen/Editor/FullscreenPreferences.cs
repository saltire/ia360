...
namespace FullscreenEditor
{
    ...
    public static class FullscreenPreferences
    {
        ...
        private static readonly GUIContent resetSettingsContent =
            new("Use Defaults", "Reset all settings to default ones");

        private static readonly GUIContent versionContent =
            new(string.Format("Version: {0} ({1:d})", pluginVersion, pluginDate));

        private static readonly GUIContent[] links =
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                using (new EditorGUI.DisabledGroupScope(EditorApplication.isCompiling))
                {
                    ...
                    var enable = GUILayout.Toggle(Integration.IsDirectiveDefined("FULLSCREEN_DEBUG"), "Debug",
                        "Button");
                    if (GUI.changed) Integration.SetDirectiveDefined("FULLSCREEN_DEBUG", enable);
                }
