...
namespace FullscreenEditor
{
    ...
    {
        private readonly byte[] backup = new byte[25];
        ...
        public void Revert()
        {
            if (!swapped) throw new Exception("Methods is not patched");
            swapped = false;

            var cursor = (byte*)pBody.ToPointer();
            for (var i = 0; i < backup.Length; i++) *cursor++ = backup[i];
        }
        ...
        {
            if (swapped) throw new Exception("Methods already patched");
            swapped = true;
            ...
            var relativeJumpAvailable = ptrDiff < 0xFFFFFFFF && ptrDiff > -0xFFFFFFFF;
            ...
            for (var i = 0; i < backup.Length; i++) backup[i] = *(ptr + i);
